...
- (void)prepareForReuse
{
    ...
    _stateView.imageView.image = nil;
...
//    _avatarView.imageView.layer.cornerRadius = 20.0;
    _avatarView.imageCornerRadius = 20.0;
    
    _stateView = [[EaseImageView alloc] init];
    [_stateView setBadgeBackgroudColor :[UIColor clearColor]];
    _stateView.imageView.backgroundColor = [UIColor clearColor];
    ...
    _lineView = [[UIView alloc] initWithFrame:CGRectMake(60, 60, SCREEN_WIDTH, 1)];
...
//    [self _setupLineViewConstraints];
    [self _setupStateImageViewConstraints];
}
...
- (void)_setupStateImageViewConstraints
{
    [self addConstraint:[NSLayoutConstraint constraintWithItem:self.stateView attribute:NSLayoutAttributeTop relatedBy:NSLayoutRelationEqual toItem:self.contentView attribute:NSLayoutAttributeTop multiplier:1.0 constant:EaseConversationCellPadding]];
    [self addConstraint:[NSLayoutConstraint constraintWithItem:self.stateView attribute:NSLayoutAttributeHeight relatedBy:NSLayoutRelationEqual toItem:self.contentView attribute:NSLayoutAttributeHeight multiplier:0.5 constant:-EaseConversationCellPadding]];
    [self addConstraint:[NSLayoutConstraint constraintWithItem:self.stateView attribute:NSLayoutAttributeRight relatedBy:NSLayoutRelationEqual toItem:self.timeLabel attribute:NSLayoutAttributeLeft multiplier:1.0 constant:-EaseConversationCellPadding]];
    [self addConstraint:[NSLayoutConstraint constraintWithItem:self.stateView attribute:NSLayoutAttributeWidth relatedBy:NSLayoutRelationEqual toItem:self.avatarView attribute:NSLayoutAttributeHeight multiplier:1.0 constant:0]];
}
...
- (void)_setupTitleLabelConstraints
{
    ...
    [self addConstraint:[NSLayoutConstraint constraintWithItem:self.titleLabel attribute:NSLayoutAttributeRight relatedBy:NSLayoutRelationEqual toItem:self.stateView attribute:NSLayoutAttributeLeft multiplier:1.0 constant:-5]];
...
- (void)setModel:(id<IConversationModel>)model
{
    ...
    if (self.showAvatar) {
        if ([_model.avatarURLPath length] > 0){
            [self.avatarView.imageView sd_setImageWithURL:[NSURL URLWithString:_model.avatarURLPath] placeholderImage:[UIImage imageNamed:@"home_默认"]];
        } else {
            self.avatarView.image = [UIImage imageNamed:@"home_默认"];
