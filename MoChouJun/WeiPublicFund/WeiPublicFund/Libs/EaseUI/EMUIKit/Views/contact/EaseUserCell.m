...
- (void)setModel:(id<IUserModel>)model
{
    ...
    self.titleLabelColor = [UIColor colorWithHexString:@"#191919"];
...
//    _avatarView.imageCornerRadius = _avatarView.width*0.5;
}

- (void)setBuddy:(PSBuddy *)buddy
{
    _buddy = buddy;
    if ([_buddy.nickName length] > 0) {
        self.titleLabel.text = _buddy.reviewName;
    }
    else{
        self.titleLabel.text = _buddy.userName;
    }
    
    if ([_buddy.avatar length] > 0){
        [self.avatarView.imageView sd_setImageWithURL:[NSURL URLWithString:_buddy.avatar] placeholderImage:nil];
    }
    else {
