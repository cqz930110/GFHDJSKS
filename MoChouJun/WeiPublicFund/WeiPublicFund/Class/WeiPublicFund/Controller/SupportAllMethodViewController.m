...
@interface SupportAllMethodViewController ()<UITableViewDataSource,UITableViewDelegate,JDpayViewControllerDelegate,SupportAllNumberTableViewCellDelegate,ExpressAddressViewControllerDelegate,ProjectReturnHeaderViewDelegate>
...
@property (strong, nonatomic) SupportReturnMamager *supportReturnmanager;
@end
...
static NSString *const supportAddress = @"SupportAddressTableViewCell";
...
- (void)viewDidLoad {
    ...
    if(_supportArr.count)
    {
        _supportReturnmanager = [[SupportReturnMamager alloc] init];
        _supportReturnmanager.supportReturns = _supportArr;
...
- (void)setTableViewInfo
{
    ...
    [_supportAllTableView registerNib:[UINib nibWithNibName:supportAddress bundle:nil] forCellReuseIdentifier:supportAddress];
    if (!_canReturn) {
        ...
        _supportAllAmountLab.text = [NSString stringWithFormat:@"¥ %.2f",_support.supportAmount];
        _supportCount = @"1";
        [_supportAllTableView selectRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:2] animated:NO scrollPosition:UITableViewScrollPositionNone];
    }
...
{
    if (section == 0 && _supportReturnmanager.supportReturns.count > 1)
    {
        return 44.0;
    }else if (section == 2) {
        return 15;
...
- (void)projectReturnCellOpenStateChanged:(ProjectReturnHeaderView *)headerView
{
    ...
    [_supportAllTableView reloadSections:indexSet withRowAnimation:UITableViewRowAnimationAutomatic];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.section == 0) {
        ...
        if (self.selectedIndexPath != 0) {
            ...
            cell.showType = YES;
            cell.supportAmountTextField.text = @"";
        }
        if (!_canReturn) {
            cell.showType = NO;
        }
        ...
        SupportAddressTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:supportAddress forIndexPath:indexPath];
        cell.selectionStyle = UITableViewCellSelectionStyleNone;
...
- (void)optionExpressAddress:(Address *)address
{
    ...
    [_supportAllTableView reloadSections:[NSIndexSet indexSetWithIndex:3] withRowAnimation:UITableViewRowAnimationAutomatic];
...
- (void)textFieldChange:(UITextField *)textField
{
    if (textField.tag == 1000) {
        _remark = (textField.text.length == 0)?@"":textField.text;
    }else{
...
- (void)requestUserBalance
{
    ...
    [self.httpUtil requestDic4MethodName:@"Account/Center" parameters:nil result:^(NSDictionary *dic, int status, NSString *msg) {
        if (status == 1 || status == 2)
        {
            ...
            [_supportAllTableView reloadSections:[NSIndexSet indexSetWithIndex:2] withRowAnimation:UITableViewRowAnimationAutomatic];
...
- (IBAction)goPayClick:(id)sender {
    ...
    if (IsStrEmpty(_supportAllAmountLab.text)) {
        [MBProgressHUD showMessag:@"您还没有选择回报方式哦～～" toView:self.view];
        return;
    }
...
- (void)payHaveRetun
{
    ...
    [MBProgressHUD showStatus:nil toView:self.view];
    [self.httpUtil requestDic4MethodName:@"Support/Add" parameters:@{@"PayType":@(_payType),@"RepayId":@(_support.Id),@"Amount":@"",@"CrowdFundId":@(_support.crowdFundId),@"Remark":[NSString encodeString:_remark],@"Count":_supportCount,@"UserAddresId":@(_userAddresId)} result:^(NSDictionary *dic, int status, NSString *msg){
        if (status == 1 || status == 2)
        {
...
{
    [MBProgressHUD showStatus:nil toView:self.view];
    [self.httpUtil requestDic4MethodName:@"Support/Add" parameters:@{@"PayType":@(_payType),@"RepayId":@(_repayId),@"Amount":_noReturnMoney,@"CrowdFundId":@(_crowdFundId),@"Remark":[NSString encodeString:_remark],@"Count":@"1",@"UserAddresId":@(_userAddresId)} result:^(NSDictionary *dic, int status, NSString *msg){
        if (status == 1 || status == 2)
        {
            _supportProjectId = [dic valueForKey:@"SupportProjectId"];
            // 如果是余额
