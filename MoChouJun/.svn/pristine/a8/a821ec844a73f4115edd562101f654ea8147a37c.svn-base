...
- (void)addFriend
{
    if(_delegate && [_delegate respondsToSelector:@selector(applyCellAddFriend:)])
    {
        [_delegate applyCellAddFriend:self];
    }
}
//
//- (void)refuseFriend
...
- (void)setApplyState:(NSInteger)applyState
{
    ...
    if (_applyState == 0)
    {
        ...
        [_addButton setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];// 176 117 211
        [_addButton setBackgroundColor:RGB(243, 167, 66)];
    }
    else if (_applyState == 1)
    {
        _addButton.userInteractionEnabled = NO;
        [_addButton setTitleColor:[UIColor lightTextColor] forState:UIControlStateNormal];
