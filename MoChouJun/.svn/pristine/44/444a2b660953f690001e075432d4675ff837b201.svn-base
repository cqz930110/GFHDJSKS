...
- (void)viewDidLoad {
    ...
    self.title = @"发起项目";
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(updateStartProjectData) name:@"UpdateStartProject" object:nil];
    [self setInfo];
    _playTime = 0;
    _second = 0;
    _minute = 0;
    ...
    _introductLab1.height =  [@"*项目详情：不得少于100字,详细说明你的项目情况" sizeWithFont:[UIFont systemFontOfSize:15] constrainedSize:CGSizeMake(SCREEN_WIDTH - 30, SCREEN_HEIGHT)].height;
    ...
    if (_myDraftDic) {
        ...
        if (IsStrEmpty([_myDraftDic objectForKey:@"Content"])) {
        ...
        _draftId = [NSString stringWithFormat:@"%@",[_myDraftDic objectForKey:@"Id"]];
...
//  清空数据
- (void)updateStartProjectData
{
    _startTitleTextField.text = @"";
    _introductTextView.text = @"";
    _introductLab1.hidden = NO;
    _introductLab2.hidden = NO;
    [_haveImages removeAllObjects];
...
- (void)draftUpload
{
    ...
    [util requestDic4MethodName:@"Draft/Save" parameters:@{@"DraftId":@(draftId),@"Name":_startTitleTextField.text,@"Content":_introductTextView.text,@"Images":imageUrl} result:^(NSDictionary *dic, int status, NSString *msg) {
