...
{
    if ([sender.titleLabel.text isEqual:@" 删除项目"]) {
        UIAlertView *alert = [[UIAlertView alloc]initWithTitle:nil message:@"确定要删除项目吗？" delegate:self cancelButtonTitle:@"取消" otherButtonTitles:@"确定", nil];
        alert.tag = sender.tag;
        [alert show];
    }else{
        UIAlertView *alert = [[UIAlertView alloc]initWithTitle:@"确定要撤回项目吗？" message:@"一旦撤回项目，筹集到的资金将退还给支持人" delegate:self cancelButtonTitle:@"取消" otherButtonTitles:@"确定", nil];
        alert.tag = sender.tag;
        [alert show];
    }
}
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    ...
    if (buttonIndex == 1) {
        ...
        [self.httpUtil requestDic4MethodName:@"CrowdFund/Delete" parameters:@{@"Id":@(myStartReturnObj.crowdFundId),@"TypeId":@(typeId)} result:^(NSDictionary *dic, int status, NSString *msg)
         {
             if (status == 1 || status == 2)
             {
                 _pageIndex = 1;
...
- (void)editProjectBtnClick:(UIButton *)sender
{
    ...
    editMyStartVC.repayCount = myStart.repayCount;
    editMyStartVC.titleStr = myStart.name;
