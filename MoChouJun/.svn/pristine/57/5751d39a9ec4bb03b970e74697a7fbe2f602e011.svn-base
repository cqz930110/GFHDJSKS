...
{
     objc_setAssociatedObject(self, @selector(timer), timer, OBJC_ASSOCIATION_ASSIGN);
}
...
- (void)asyncPlayingWithPath:(NSString *)aFilePath
                  playing:(void(^)(NSError *error ,NSTimeInterval duraiton,NSTimeInterval currentTime))playing
{
...
//        BOOL covertRet = [self ps_amrToWav:aFilePath wavSavePath:wavFilePath];
        BOOL covertRet = [self convertAMR:aFilePath toWAV:wavFilePath];
        if (!covertRet) {
            if (playing) {
                playing([NSError errorWithDomain:NSLocalizedString(@"error.initRecorderFail", @"File format conversion failed")
                                              code:EMErrorFileTypeConvertionFailure
                                          userInfo:nil],0.0,0.0);
            }
            return ;
        }
    }
    ...
    // 及时间
    [self removeTimer];
    NSTimer *timer =  [HWWeakTimer scheduledTimerWithTimeInterval:0.5 block:^(id userInfo)
                       {
                           if (playing)
                           {
                               playing(nil,[EMAudioPlayerUtil duration],[EMAudioPlayerUtil currentTime]);
                           }
    ...
    [EMAudioPlayerUtil asyncPlayingWithPath:wavFilePath
                                 completion:^(NSError *error)
     {
         ...
         [self removeTimer];
         if (playing) {
             playing(error,[EMAudioPlayerUtil duration],[EMAudioPlayerUtil currentTime]);
         }
     }];
}
