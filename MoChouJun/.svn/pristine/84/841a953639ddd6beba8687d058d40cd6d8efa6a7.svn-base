...
@property (assign,nonatomic) CGFloat applyMessageHeight;
@end
...
- (void)_imageMessageCellSelected:(id<IMessageModel>)model
{
    ...
    if ([imageBody messageBodyType] == eMessageBodyType_Image) {
        if (imageBody.thumbnailDownloadStatus == EMAttachmentDownloadSuccessed) {
            ...
            [MBProgressHUD showStatus:@"图片下载中..." toView:weakSelf.view];
            [chatManager asyncFetchMessage:model.message progress:nil completion:^(EMMessage *aMessage, EMError *error) {
                [weakSelf hideHud];
                if (!error) {
                    //发送已读回执
               ...
               [MBProgressHUD dismissHUDForView:weakSelf.view withError:@"图片下载失败..."];
            } onQueue:nil];
        }else{
            //获取缩略图
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    else
    {
        ...
        if ([model.message.ext.allKeys containsObject:@"IsSys"])
        {
            if (_conversation.conversationType == eConversationTypeChat)
            {
                ...
                {
                    // 个人回话 applyMessage
                    UITableViewCell *cell = [[UITableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"ApplyEntityCell"];
                    cell.backgroundColor = [UIColor clearColor];
                    UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(50, 0, tableView.frame.size.width - 100, 0)];
                    label.textAlignment = NSTextAlignmentCenter;
                    label.numberOfLines = 0;
                    label.font = [UIFont systemFontOfSize:12];
                    label.textColor = [UIColor grayColor];
                    
                    PSChat *chat = [ChatBuddyDAO chatBuddyWithUserame:model.message.to];
                    label.text = [NSString stringWithFormat:@"你已经添加了\"%@\",现在可以开始聊天了",chat.showName];
                    [label sizeToFit];
                    label.center = CGPointMake(SCREEN_WIDTH * 0.5, label.height * 0.5);
                    [cell addSubview:label];
                    
                    _applyMessageHeight = label.height;
                    return cell;
                }
                
            }
            else if (_conversation.conversationType == eConversationTypeGroupChat)
            {
                // 群聊
                UITableViewCell *cell = [[UITableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"ApplyEntityCell"];
                cell.backgroundColor = [UIColor clearColor];
                ...
                label.font = [UIFont systemFontOfSize:12];
                label.textColor = [UIColor grayColor];
                PSChat *chat = [ChatBuddyDAO chatBuddyWithUserame:model.nickname];
                NSString *appliName;
                if (model.isSender)
                {
                    appliName = @"你";
                ...
                else
                {
                    if (chat)
                    {
                        appliName = chat.showName;
                    }
                    else
                    {
                        appliName = model.nickname;
                    }
                }

                label.text = [NSString stringWithFormat:@"%@%@",appliName,model.text];
                [label sizeToFit];
                label.center = CGPointMake(SCREEN_WIDTH * 0.5, label.height * 0.5);
                [cell addSubview:label];
                _applyMessageHeight = label.height;
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    {
        id<IMessageModel> model = object;
        if (model.isSender && [model.message.ext.allKeys containsObject:@"IsSys"])
        {
            return _applyMessageHeight + 10;
        }
                ...
                if ([messageId isEqualToString:model.message.messageId]) {
                    ...
                    else
                    {
                        ...
                        if (error && error.errorCode == EMErrorMessageContainSensitiveWords)
                        {
                            ...
                            [self showHint:@"你的消息包含不当言论" yOffset:-frame.size.height + 50];
                        }
                    }
