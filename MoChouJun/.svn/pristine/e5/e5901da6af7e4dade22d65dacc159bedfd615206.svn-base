...
+ (NSArray*)propertyKeysWithClassName:(Class)clazz//EMChatManager
...
+ (id)reflectDataWithClassName:(NSString *) calssName otherObject:(NSObject*)dataSource {
    ...
    for (NSString *key in [self propertyKeysWithClassName:class]) {
        ...
        if ([dataSource isKindOfClass:[NSDictionary class]])
        {
            ...
            ret = ([dataSource valueForKey:conventStr]==nil && [dataSource valueForKey:key] == nil)?NO:YES;
        }
        else
        {
            ret = [dataSource respondsToSelector:NSSelectorFromString(conventStr)] && [dataSource respondsToSelector:NSSelectorFromString(key)];
        }
...
+ (NSArray *)reflectDataWithClassName:(NSString *) calssName otherObject:(NSObject*)dataSource isList:(BOOL)isList{
    if(!isList)
